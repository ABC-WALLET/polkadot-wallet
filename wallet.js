...
const polkadotTransfer = async (senderPKEY, recipientPKEY, txAmount) => {
    const wsProvider = new WsProvider('wss://westend-rpc.polkadot.io');
    const api = await ApiPromise.create({ provider: wsProvider });
...
polkadotTransfer("sponsor file start tube scout gallery dance volcano neutral flavor senior admit","true crime ensure pig unique odor clarify access kid ship era exist",1)